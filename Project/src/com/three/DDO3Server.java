...
public class DDO3Server implements Runnable {
	...
	private static int DDO3Port = 1009;
	private int FEPort = 7825;
	
    ExecutorService exec = Executors.newFixedThreadPool(10);
    
    public DDO3Server() throws IOException 
    {
		super();
			...
			String specialization, String location) 
	{
		try{	
			...
			synchronized(this) {
				if(DDO3Server.database.containsKey(key)){
					records=DDO3Server.database.get(key);
					records.add(recobj);
					DDO3Server.database.put(key, records);
				}
				else{
	...
	public static void main(String[] args) throws Exception {
		...
		try{
			...
			ddo = new DatagramSocket(DDO3Port);
			while(true){
				String bloop = "";
				byte[] buffer = new byte[1000];
