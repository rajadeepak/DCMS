...
public class MTL3Server implements Runnable {
			...
			String specialization, String location) 
	{
		try{	
			...
			synchronized(this) {
				if(MTL3Server.database.containsKey(key)){
					records=MTL3Server.database.get(key);
					records.add(recobj);
					MTL3Server.database.put(key, records);
				}
				else{
	...
	public static void main(String[] args) throws Exception {
		...
		try{
			ddo = new DatagramSocket(MTL3Port);
			while(true){
				String bloop = "empty";
				byte[] buffer = new byte[1000];
