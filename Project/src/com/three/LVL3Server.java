...
public class LVL3Server implements Runnable{
	...
	private static int LVL3Port = 1008;
	private int DDO3Port = 1009;
	private int FEPort = 7825;
	ExecutorService exec = Executors.newFixedThreadPool(10);
	
	public LVL3Server() throws RemoteException,IOException
	{
		super();
    ...
    public String createTRecord(String ManagerID, String firstName, String lastName, String address, String phone, String specialization, String location) 
	{
		try{	
			...
			synchronized(this) {
				if(LVL3Server.database.containsKey(key)){
					records=LVL3Server.database.get(key);
					records.add(recobj);
					LVL3Server.database.put(key, records);
				}
				else{
	...
	public static void main(String[] args) throws Exception {
		...
		try{
			...
			ddo = new DatagramSocket(LVL3Port);
			while(true){
				String bloop = "";
				byte[] buffer = new byte[1000];
