    ...
    public static class MyPacketHandler extends PacketHandler{
		...
		public void onReliablePacketReceived(byte[] data) {
			...
			else if(rep.contains("createSRecord"))
			{
				...
				bloop = LVL3Server.getInstance().createSRecord(parts[1], parts[2], parts[3], parts[4], parts[5], parts[6]);
			}
			...
			else if(rep.contains("editRecord"))
			{
				...
				bloop = LVL3Server.getInstance().editRecord(parts[1], parts[2], parts[3], parts[4]);
			}
			...
			else if(rep.contains("GRCMethod"))
			{
				...
				bloop = LVL3Server.getInstance().getRecordCounts(parts[1]);
			}
			...
			else if(rep.contains("TRMethod"))
			{
				...
				bloop = LVL3Server.getInstance().transferRecord(parts[1], parts[2], parts[3]);
			}
		...
		try {
			...
			Future<String> mtlResponse = exec.submit(new Callable<String>() {
				...
				public String call() throws Exception {
					...
					DatagramPacket request = new DatagramPacket(message, message.length, aHost, MTL3Port);
					ds.send(request);
			...
			Future<String> ddoResponse = exec.submit(new Callable<String>() {
				...
				public String call() throws Exception {
					DatagramSocket ds = new DatagramSocket();
					InetAddress aHost = InetAddress.getByName("localhost");
					DatagramPacket request = new DatagramPacket(message, message.length, aHost, DDO3Port);
					ds.send(request);
