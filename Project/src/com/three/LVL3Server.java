...
public class LVL3Server {
    ...
    private static LVL3Server serverInstance = new LVL3Server();
    
    private LVL3Server() {
		super();
		try {
			logger = new LogManager("lvl-3-server.log");
		} catch (IOException e) {
			e.printStackTrace();
		}
    ...
    public static LVL3Server getInstance() {
    ...
    public static class MyPacketHandler extends PacketHandler{
		...
		public void onReliablePacketReceived(byte[] data) {
			...
			if(rep.equals("getRecordCounts"))
				bloop = "LVL "+ String.valueOf(LVL3Server.getInstance().database.size()); 
			...
			else if(rep.contains("transferRecord"))
			{
				...
				if(parts[2].startsWith("TR"))
					bloop = LVL3Server.getInstance().createTRecord(parts[1], parts[3], parts[4], parts[5], parts[6], parts[7], parts[8]);
				else
					bloop = LVL3Server.getInstance().createSRecord(parts[1], parts[3], parts[4], parts[5], parts[6], parts[7]);
			}
			...
			else if(rep.contains("createTRecord"))
			{
				String bleep = rep;
				String parts[] = bleep.split("::");
				bloop = LVL3Server.getInstance().createTRecord(parts[2], parts[3], parts[4], parts[5], parts[6], parts[7], parts[8]);
				if(parts[0].contains("true"))
				{
					String str = rep.replace("true", "false");
					LVL3Server.getInstance().forwardRequest(str,LVL2Port);
					LVL3Server.getInstance().forwardRequest(str,LVL1Port);
				}
			...
			else if(rep.contains("createSRecord"))
			{
				String bleep = rep;
				String parts[] = bleep.split("::");
				bloop = LVL3Server.getInstance().createSRecord(parts[2], parts[3], parts[4], parts[5], parts[6], parts[7]);
				if(parts[0].contains("true"))
				{
					String str = rep.replace("true", "false");
					LVL3Server.getInstance().forwardRequest(str,LVL2Port);
			...
			else if(rep.contains("editRecord"))
			{
				String bleep = rep;
				String parts[] = bleep.split("::");
				bloop = LVL3Server.getInstance().editRecord(parts[2], parts[3], parts[4], parts[5]);
				if(parts[0].contains("true"))
				{
					String str = rep.replace("true", "false");
					LVL3Server.getInstance().forwardRequest(str,LVL2Port);
			...
			else if(rep.contains("GRCMethod"))
			{
				String bleep = rep;
				String parts[] = bleep.split("::");
				bloop = LVL3Server.getInstance().getRecordCounts(parts[2]);
				if(parts[0].contains("true"))
				{
					String str = rep.replace("true", "false");
					LVL3Server.getInstance().forwardRequest(str,LVL2Port);
			...
			else if(rep.contains("TRMethod"))
			{
				String bleep = rep;
				String parts[] = bleep.split("::");
				bloop = LVL3Server.getInstance().transferRecord(parts[2], parts[3], parts[4]);
				if(parts[0].contains("true"))
				{
					String str = rep.replace("true", "false");
					LVL3Server.getInstance().forwardRequest(str,LVL2Port);
			...
			try {
				LVL3Server.getInstance().server.getConnectedClients().get(0).sendReliablePacket(bloop.getBytes("UTF-8"));
		...
		try {
			...
			Future<String> ddoResponse = exec.submit(new Callable<String>() {
				...
				public String call() throws Exception {
					DatagramSocket ds = new DatagramSocket();
					InetAddress aHost = InetAddress.getByName("localhost");
					DatagramPacket request = new DatagramPacket(message, message.length, aHost, DDOPort3);
					ds.send(request);
	...
	public static void main(String[] args) throws Exception {
		...
		try{
			...
			ddo = new DatagramSocket(LVLPort3);
			while(true){
				String bloop = "";
				byte[] buffer = new byte[1000];
				...
				if(data(buffer).toString().equals("getRecordCounts"))
					bloop = "LVL "+ String.valueOf(LVL3Server.getInstance().getSize()) + ", "; 
	...
	private void startServer() {
		try {
			executor.scheduleAtFixedRate(new HeartbeatGenerator("LVL3"), 1 , 5, TimeUnit.SECONDS);
			exec.execute(new ElectionTriggerListener("LVL3"));
		} catch (NamingException | JMSException | IOException e) {
			e.printStackTrace();
		}
		try {
			server = new RUDPServer(LVL3Port);
			server.setPacketHandler(MyPacketHandler.class);
			server.start();
		}
		catch(SocketException e) {
			System.out.println("Port 8003 is occupied. Server couldn't be initialized.");
			System.exit(-1);
		}
	}
