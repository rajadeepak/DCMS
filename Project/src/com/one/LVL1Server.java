...
public class LVL1Server implements Runnable {
	...
	private static int LVL1Port = 1002;
	...
	public LVL1Server() throws RemoteException,IOException
	{
		super();
    ...
    public String createTRecord(String ManagerID, String firstName, String lastName, String address, String phone, String specialization, String location) 
	{
		try{	
			...
			synchronized(this) {
				if(LVL1Server.database.containsKey(key)){
					records=LVL1Server.database.get(key);
					records.add(recobj);
					LVL1Server.database.put(key, records);
				}
				else{
	...
	public void run() {
		...
		try{
			LVL1Server obj = new LVL1Server();
			ddo = new DatagramSocket(LVL1Port);
			while(true){
				String bloop = "";
				byte[] buffer = new byte[1000];
