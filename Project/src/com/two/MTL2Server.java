...
public class MTL2Server implements Runnable{
	...
	private static int MTL2Port= 1004;
	...
	public MTL2Server() throws IOException 
	{
		super();
			...
			String specialization, String location) 
	{
		try{	
			...
			synchronized(this) {
				if(MTL2Server.database.containsKey(key)){
					records=MTL2Server.database.get(key);
					records.add(recobj);
					MTL2Server.database.put(key, records);
				}
				else{
	...
	public static void main(String[] args) throws Exception {
		...
		try{
			ddo = new DatagramSocket(MTL2Port);
			while(true){
				String bloop = "empty";
				byte[] buffer = new byte[1000];
