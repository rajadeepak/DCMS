...
public class MTL2Server {
	...
	private static MTL2Server serverInstance = new MTL2Server();
	
	private MTL2Server() {
		super();
		try {
			logger = new LogManager("mtl-2-server.log");
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static MTL2Server getInstance() {
    ...
    public static class MyPacketHandler extends PacketHandler{
		...
		public void onReliablePacketReceived(byte[] data) {
			...
			if(rep.equals("getRecordCounts"))
				bloop = "MTL "+ String.valueOf(MTL2Server.getInstance().database.size()); 
			...
			try {
				MTL2Server.getInstance().server.getConnectedClients().get(0).sendReliablePacket(bloop.getBytes("UTF-8"));
	...
	private void startServer() {
		try {
			executor.scheduleAtFixedRate(new HeartbeatGenerator("MTL2"), 1 , 5, TimeUnit.SECONDS);
			exec.execute(new ElectionTriggerListener("MTL2"));
